...
class GreedySearch(StackFrontier):
    def remove(self, cost):
        if self.empty():
            raise Exception('Frontier is already empty')
        else:
            best_neighbor = set()
            best_neighbor_cost = 0
            for node in self.frontier:
...
#UCS Search
class UniformSearch(StackFrontier):
    def remove(self, cost):
        if self.empty():
            raise Exception('Frontier already empty!')
        else:
...
class Maze():
    ...
    def solve(self, show_steps):
        ...
        if self.algorithm == 1:
            ...
            frontier = GreedySearch()
        elif self.algorithm == 5:
            frontier = UniformSearch()
...
algo_list = DropDown(600,200,190,40, WHITE, (100,200,255), BTN_TEXT, ['Pick an Algorithm','DFS','BFS','A* Search','UCS Search', 'Greedy Search'])
solve_button = pygame.Rect(600,475,100,50)
checkbox = Checkbox(pygame.Rect(600,425,25,25), '', BTN_TEXT)
        ...
        if homepage:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if solve_button.collidepoint(mousePos):
                    ...
                    #IF TOO LITTLE WALLS
                    if sum(sum(row) for row in board) < 25 and algorithm in (2,3,4,5):
